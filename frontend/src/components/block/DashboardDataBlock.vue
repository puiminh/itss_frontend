<template>
    <!-- transition duration-300 ease-in-out -->
    <div :class="[select? '' :'grayscale ', ' px-8 relative w-full h-52 bg-cover bg-center group rounded-lg overflow-hidden shadow-md  cursor-pointer hover:grayscale-0']">
        <div :class="['absolute inset-0 bg-opacity-75 transition duration-300 ease-in-out ', color]"></div>
        <div class="relative w-full h-full px-4 sm:px-6 lg:px-4 flex items-center">
        <div>
            <div class="text-white text-lg flex space-x-2 items-center">
            <div class="bg-white rounded-md p-2 flex items-center text-green-600">
                <slot>
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                        <path d="M4.5 6.375a4.125 4.125 0 118.25 0 4.125 4.125 0 01-8.25 0zM14.25 8.625a3.375 3.375 0 116.75 0 3.375 3.375 0 01-6.75 0zM1.5 19.125a7.125 7.125 0 0114.25 0v.003l-.001.119a.75.75 0 01-.363.63 13.067 13.067 0 01-6.761 1.873c-2.472 0-4.786-.684-6.76-1.873a.75.75 0 01-.364-.63l-.001-.122zM17.25 19.128l-.001.144a2.25 2.25 0 01-.233.96 10.088 10.088 0 005.06-1.01.75.75 0 00.42-.643 4.875 4.875 0 00-6.957-4.611 8.586 8.586 0 011.71 5.157v.003z"></path>
                    </svg>
                </slot>

            </div>
            <p>{{ data }}</p>
            </div>
            <h3 class="text-white text-3xl mt-2 font-bold">
                {{ total }}
            </h3>
            <h3 class=" text-lg mt-2 text-gray-200 ">
                {{ new }} new {{data}}s this week
            </h3>
        </div>
        </div>
        <div class="hidden bg-green-600 bg-blue-600 bg-yellow-600 bg-pink-600 grayscale grayscale-0">
        </div>
    </div> 
</template>

<script>
export default {
    data() {
        return {

        }
    },
    props: {
        color: {
            default: 'bg-green-600'
        },
        data: {
            default: ''
        },
        total: {
            default: ''
        },
        new: {
            default: ''
        },
        select: {
            default: false,
        }
    }
}


</script>