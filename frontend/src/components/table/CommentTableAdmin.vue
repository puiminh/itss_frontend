        ...
        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th @click="sort('content')" scope="col" :class="['px-6 py-3 cursor-pointer hover:underline border-b-2 border-transparent ',currentSort == 'content' ? 'border-green-700' : '']">
                        Content
                    </th>
                    <th @click="sort('user_id')" scope="col" :class="['px-6 py-3 cursor-pointer hover:underline border-b-2 border-transparent ',currentSort == 'user_id' ? 'border-green-700' : '']">
                        Author
                    </th>
                    <th @click="sort('course_id')" scope="col" :class="['px-6 py-3 cursor-pointer hover:underline border-b-2 border-transparent ',currentSort == 'course_id' ? 'border-green-700' : '']">
                        Course
                    </th>
...
import DeleteConfirm from '../modal/DeleteConfirm.vue';
    export default {
        ...
        deleteMethod(data) {
            axios.delete(`/comments/${data.id}/${this.getUser.id}`)
                .then(response => {
                this.data = this.data.filter(child => child.id != data.id);
            ...
            sortedData:function() {
                ...
                if (this.keyword) {
                    array = this.data.filter((e)=> {
                      return e.content?.toLowerCase().includes(this.keyword.toLowerCase())
                    })
                } else {
