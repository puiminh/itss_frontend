            ...
            <TransitionGroup appear name="list" tag="tbody"  mode="out-in">
                <tr v-for="i in sortedData" :key="i.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4 flex gap-4">
                        <RouterLink :to="`/course/${i.id}`" type="button" data-id="' + row.id + '" class="hover:text-yellow-400">
                        ...
                        <RouterLink :to="`/course/${i.id}/edit`" data-id="' + row.id + '" class="editButton hover:text-green-500">
...
import { useUserStore } from '../../stores/user';
    ...
    export default {
        ...
        async mounted() {
            try {
                    const response = await axios.get(`/users/bookmarked/courses_collections/${this.getUser.id}`)
