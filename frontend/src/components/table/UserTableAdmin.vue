          ...
          <TransitionGroup appear name="list" tag="tbody"  mode="out-in">
              <tr v-for="i in sortedData" :key="i.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                  ...
                  <td class="px-6 py-4 flex gap-4">
                      <RouterLink :to="`/user/view/${i.id}`" type="button" data-id="' + row.id + '" class="hover:text-yellow-400">
...
<!--     
                      <RouterLink :to="`/user/view/${i.id}/edit`" data-id="' + row.id + '" class="editButton hover:text-green-500">
  ...
  export default {
      ...
      computed:{
          ...
          sortedData:function() {
              let array = []
              if (this.keyword) {
                  array = this.dataChildren.filter((e)=> {
      ...
      mounted() {
        axios.get('/users').then((res)=>{
            ...
            this.dataChildren = res.data.data
        })
      },
