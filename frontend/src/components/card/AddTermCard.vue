<template>
    <div ref="wrap" :class="['bg-white rounded-md flex flex-col divide-y ', errorTerm ? ' border-4 border-red-600': '']">
        <div class="flex justify-between pt-4 px-8 pb-2">
            ...
            <div @click="deleteTerm" class="hover:text-red-500 cursor-pointer">
...
export default {
    ...
    methods: {
        ...
        deleteTerm() {
            this.vocab.word = null;
            this.vocab.define = null;
            gsap.to(this.$refs.wrap,{
                duration: 0.5,
                right: '-500px',
                opacity: 0,
    ...
    mounted() {
        if (this.predata) {
            this.vocab = this.predata;
            this.imageLink = this.vocab.link;
            this.recordLink = this.vocab.link;
        }
    }
}
</script>